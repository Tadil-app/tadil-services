...
const pages: Array<
  [string, string | undefined, () => Promise<{ default: Component }>]
> = [
  [
  ...
  [
    'extras',
    'extras.read',
    () => import('@/views/extras/ExtrasListingView.vue'),
  ],
  [
    'models',
    'models.read',
    () => import('@/views/models/ModelsListingView.vue'),
  ],
  [
    'tailors',
    'tailors.read',
    () => import('@/views/users/TailorsListView.vue'),
  ],
  [
    'couriers',
    'couriers.read',
    () => import('@/views/users/CouriersListView.vue'),
  ],
  [
    'customers',
    'customers.read',
    () => import('@/views/users/CustomersListView.vue'),
  ],
  [
    'login-requests',
    'login_requests.read',
    () => import('@/views/users/LoginRequestsView.vue'),
  ],
  [
    'payout-requests',
    'payouts.read',
    () => import('@/views/users/PayoutRequestsView.vue'),
  ],
  [
    'orders',
    'orders.read',
    () => import('@/views/orders/OrdersManagementView.vue'),
  ],
  [
    'staff',
    'staff.read',
    () => import('@/views/staff/StaffManagementView.vue'),
  ],
  [
    'roles',
    'roles.read',
    () => import('@/views/staff/RolesManagementView.vue'),
  ],
...
router.beforeEach(async (to) => {
  if (authState.user && !to.meta.guest)
    await (await import('@/auth')).refreshAuth();
  if (to.meta.guest) {
    if (authState.user && to.path === '/login') return '/';
    return true;
  }
  if (!authState.user)
